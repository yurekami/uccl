...
class RDMAChannelImpl {
 public:
  ...
  virtual void connectQP(struct ibv_qp* qp,
  ...
  virtual bool poll_once(struct ibv_cq_ex* cq_ex,
                         std::vector<CQMeta>& cq_datas,
                         uint32_t channel_id) = 0;

  // Post receive work request
  virtual void lazy_post_recv_wr(struct ibv_qp* qp,
                                  uint32_t threshold,
  ...
  virtual void setDstAddress(struct ibv_qp_ex* qpx,
                              struct ibv_ah* ah,
...
// Factory function declaration (implementation in rdma_channel.h after including impl headers)
